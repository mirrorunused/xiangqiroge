...
(() => {
  ...
  async function win(text) {
    ...
    const rewardText = state.mode === "random" ? "获得本关随机道具后继续进军。" : "选择一个奖励继续进军。";
  ...
  async function nextLevel() {
    if (state.phase !== "rewarded" && !(["normal", "random"].includes(state.mode) && state.phase === "play" && state.level <= 5)) return;
    state.level += 1;
  ...
  function bind() {
    ...
    runtime.bind("giveUpBtn", "give-up", () => { closeMore(); return state.phase === "play" && lose(); });
  }
