...
function normalize(raw: any) {
  ...
  s.mode = ["normal", "rebel", "random", "quick"].includes(s.mode) ? s.mode : "normal";
  ...
  s.bestRecords = Object.assign({ normal: s.bestRecord, rebel: { level: 1, score: 0 }, random: { level: 1, score: 0 } }, s.bestRecords || {});
...
function outcome(state: any) {
  ...
  if (state.mode === "random") {
    if (!red.length) return { status: "lose", text: "红方棋子已全部被吃，随机棋征程结束" };
...
async function settleWin(ctx: any, state: any, args: any, secret: string) {
  ...
  if (state.mode === "quick") {
    state.score = score(state) + 1000;
    state.phase = "quick-won";
    state.message = String(args.text || "快速模式胜利");
    state.pendingRewards = [];
    state.lastSettlement = { base: 1000, banner: 1, scoreMult: 1, points: 1000, formula: "快速模式固定胜利奖励=1000" };
...
function addTempo(state: any) { if (state.mode !== "normal" || ownedCount(state, "tempo") > 0) return false; pushItem(state, { id: "tempo", name: "双步虎符", rarity: "red", text: "每回合红方可走两步，累计 25 道具后失效。", value: 1 }); return true; }
function hasTempo(state: any) { return state.mode === "normal" && ((state.talents?.outerItems || []).some((i: any) => i.id === "tempo") || (isFullEnemy(state.level) && activeItems(state).some((i: any) => i.id === "tempo") && state.items.length < 25)); }
...
function unlockTempo(state: any) { if (state.mode !== "normal") return; const t = state.talents || (state.talents = {}); if (!t.outerTempoOffer) { t.outerTempoOffer = true; t.outerTempoNotice = true; } }
